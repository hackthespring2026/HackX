...
def send_alert_to_flask(alert_type, details=None):
    try:
        requests.post(ALERT_API, json={
            "type": alert_type,
            "details": details or {}
        }, timeout=0.2)
    except Exception as e:
        print("Flask alert send failed:", e)
    ...
    if person_detected and hand_point and pocket_zone:
        ...
        if inside(DESK_BOX, hand_point):
            ...
            send_alert_to_flask(
                alert_type="THEFT_SUSPECTED",
                details={
                    "flow": "desk_to_pocket or drawer_to_pocket",
                    "drawer_open": drawer_open
                }
            )
