...
function Map({
    ...
    // Generate a stable unique key per mount to prevent "Map container is being reused" errors
    const mapId = useId()
    const [mapKey, setMapKey] = useState(() => mapId + "-" + Date.now())

    // Force remount on navigation by tracking mount cycle
    useEffect(() => {
        return () => {
            // On unmount, invalidate the key so next mount creates fresh DOM
            setMapKey(mapId + "-" + Date.now())
        }
    }, [mapId])
...
function MapLocatePulseIcon() {
    return (
        <div className="relative flex size-3 rounded-full">
            <div className="bg-blue-500 absolute inline-flex size-full animate-ping rounded-full opacity-75" />
            <div className="bg-blue-500 relative inline-flex size-3 rounded-full ring-2 ring-white" />
        </div>
    )
}
...
function MapLocateControl({
    ...
    function startLocating(setView = true) {
        setIsLocating(true)
        map.locate({ setView, maxZoom: map.getMaxZoom(), watch })
    ...
    // Auto-start location tracking on mount (without panning the map)
    useEffect(() => {
    ...
    return (
        <MapControlContainer className={cn(position, className)}>
            <Button
                ...
                onClick={location ? stopLocating : () => startLocating(true)}
                disabled={isLocating}
                title={
                    isLocating
                        ? "Locating..."
            ...
            {location && (
                <MapMarker position={location} icon={<MapLocatePulseIcon />} iconAnchor={[6, 6]} />
            )}
        </MapControlContainer>
    )
}
